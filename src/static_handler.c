...
void update_cache(const char *path, const char *content, size_t size, time_t last_modified) {
    ...
    if (entry == NULL) {
        if (cache_count < CACHE_SIZE) {
            ...
            entry = &cache[0]; // Replace the first entry (simple LRU strategy)
        }
...
void serve_static_file(const char *url, SOCKET client_socket) {
    ...
    if (cached != NULL && stat(path, &st) == 0 && cached->last_modified == st.st_mtime) {
        // Serve from cache
