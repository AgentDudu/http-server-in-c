...
void handle_request(const char *request, SOCKET client_socket) {
    ...
    // Get client IP from `server.c` (Modify if needed)
    extern char client_ip[32]; // Define in `server.c`
    extern double request_duration; // Define in `server.c`

    log_request(method, url, client_ip, request_duration);
    ...
    if (sscanf(request, "%*[^C]Cookie: session_id=%36s", session_id) == 1) {
        log_request("Cookie detected", session_id, client_ip, request_duration);
    }
